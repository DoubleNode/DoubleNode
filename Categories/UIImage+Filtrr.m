...
- (UIImage*)createImageFromContext:(CGContextRef)cgctx
...
- (UIImage*)createImageFromPixels:(unsigned char*)outData
               ...
               Callback:(RGBA(^)(int r, int g, int b, int a))fn
{
    ...
    if (data != NULL)
    {
        ...
        {
            RGBA newColors = fn(data[i + shiftIn.one], data[i + shiftIn.two], data[i + shiftIn.three], data[i + shiftIn.four]);
...
- (unsigned char*)convolveRaw:(NSArray*)kernel
                       InData:(unsigned char*)inData
                       OuData:(unsigned char*)outData
                       Height:(uint)_height
                        Width:(uint)_width
{
